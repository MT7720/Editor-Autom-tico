...
try:
    ...
    # Isso sera tratado de forma elegante no aplicativo
...
def main():
    if not __name__ == "__main__": print("Este script deve ser executado como 'python main.py'"); return
    if platform.system() == "Windows":
        try: from ctypes import windll; windll.shcore.SetProcessDpiAwareness(1)
        except Exception as e: print(f"Não foi possível definir a conscientização de DPI: {e}")
    if not os.path.exists("video_processing_logic.py"):
        ...
        root = tk.Tk(); root.withdraw(); Messagebox.show_error(error_msg, "Arquivo Faltando"); return
    try:
        app = VideoEditorApp()
        app.root.mainloop()
    except Exception as e:
        logging.getLogger().critical(f"Ocorreu um erro fatal: {e}", exc_info=True)
        try:
            root = tk.Tk(); root.withdraw()
            Messagebox.show_error(f"Ocorreu um erro fatal e o aplicativo precisa fechar:\n\n{e}", "Erro Fatal")
        except: pass
